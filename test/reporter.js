module.exports = function (jasmine) {
  ...
  jasmineNode.TerminalReporter = function (config) {
    ...
    this.color_ = config.color ? jasmineNode.NoColors : jasmineNode.NoColors;
  ...
  jasmineNode.TerminalVerboseReporter.prototype = {
    ...
    buildMessagesFromResults_(messages, results) {
      var element,
        specResult,
        specIndentSpaces,
        msg = '';
      ...
      for (var i = 0; i < results_length; i++) {
        ...
        if (element.type === 'spec') {
          ...
          if (specResult.result === 'passed') {
            msg = this.stringWithColor_(
              this.indentMessage_(element.name, specIndentSpaces),
              this.color_.pass(),
            );
          } else {
            msg = this.stringWithColor_(
    ...
    indentMessage_(message, indentCount) {
      var _indent = '';
