describe('jsdoc/util/markdown', () => {
  ...
  describe('getParser', () => {
    ...
    it('should not apply formatting to inline tags', () => {
      ...
      expect(parser('{@link MyClass#_x} and {@link MyClass#_y}')).toBe('<p>{@link MyClass#_x} and {@link MyClass#_y}</p>');
    });

    it('should not automatically convert HTTP/HTTPS URLs to links', () => {
      var parser = markdown.getParser();

      expect(parser('Visit {@link http://usejsdoc.com}.')).toBe('<p>Visit {@link http://usejsdoc.com}.</p>');
      expect(parser('Visit {@link https://google.com}.')).toBe('<p>Visit {@link https://google.com}.</p>');
    });
    ...
    describe('syntax highlighter', () => {
      ...
      it(
        'should log an error if the `highlight` module does not assign a method to ' +
          '`exports.highlight`',
        () => {
          spyOn(logger, 'error');
