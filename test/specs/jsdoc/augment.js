import * as augment from '../../../lib/jsdoc/augment';
...
describe('jsdoc/augment', () => {
  ...
  describe('augmentAll', () => {
    xit('should call all other methods that the module exports', () => {
      var docSet = jasmine.getDocSetFromFile('test/fixtures/mixintag2.js', null, null, false);
      var methodNames = Object.keys(augment).filter(name => name !== 'augmentAll');
    ...
    it('should work when a class extends another class that implements an interface', () => {
      ...
      open = docSet.getByLongname('EncryptedSocket#open').filter(d => !d.ignore);
