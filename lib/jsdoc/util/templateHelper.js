...
import env from '../env';
import { replaceInlineTags } from '../tag/inline';
import logger from '../util/logger';
import { SCOPE, shorten } from '../name';
 ...
 */
export const getUniqueId = makeUniqueId;
 ...
 */
export function getSignatureTypes(d, cssClass) {
  var types = [];
 ...
 */
export function getSignatureParams(d, optClass) {
  var pnames = [];
 ...
 */
export function getSignatureReturns(d, cssClass) {
  var returnTypes = [];

  if (d.yields || d.returns) {
    (d.yields || d.returns).forEach((r) => {
...
export function prune(data) {
  ...
  if (!env.opts.access || (env.opts.access && env.opts.access.indexOf('all') === -1)) {
    ...
    if (
      !env.opts.private &&
      (!env.opts.access || (env.opts.access && env.opts.access.indexOf('private') === -1))
    ) {
 ...
 */
export { longnamesToTree } from '../name';
