    ...
    case 'string':
      ...
      if (highlighterPath) {
        ...
        if (typeof highlighter !== 'function') {
          logger.error(
            'The syntax highlighting module "%s" does not assign a method ' +
        ...
        logger.error(
          'Unable to find the syntax highlighting module "%s". Using the ' +
...
function getParseFunction(parserName, conf) {
  ...
  switch (parserName) {
    ...
    default:
      logger.error('Unrecognized Markdown parser "%s". Markdown support is disabled.', parserName);
