...
import { applyNamespace, LONGNAMES, resolve, SCOPE } from './name';
import { Syntax } from './src/syntax';
import { isFunction } from './src/astnode';
import defaultDictionary from './tag/dictionary';
import { Tag } from './tag';
...
function fixDescription(docletSrc, meta) {
  ...
  if (!/^\s*@/.test(docletSrc) && docletSrc.replace(/\s/g, '').length) {
    isClass =
      meta.code &&
      (meta.code.type === Syntax.ClassDeclaration || meta.code.type === Syntax.ClassExpression);
 ...
 */
export function _replaceDictionary(dict) {
  dictionary = dict;
 ...
 */
export function Doclet(docletSrc, meta) {
  var newTags = [];
...
Doclet.prototype.addTag = function (title, text) {
  var tagDef = dictionary.lookUp(title);
  var newTag = new Tag(title, text, this.meta);
...
function removeGlobal(longname) {
  var globalRegexp = new RegExp(`^${LONGNAMES.GLOBAL}\\.?`);
...
Doclet.prototype.setMemberof = function (sid) {
   ...
   * @type {string}
   */
  this.memberof = removeGlobal(sid).replace(/\.prototype/g, SCOPE.PUNC.INSTANCE);
};
...
Doclet.prototype.setScope = function (scope) {
  var errorMessage;
  var filepath;
  var scopeNames = _.values(SCOPE.NAMES);

  if (scopeNames.indexOf(scope) === -1) {
    filepath = getFilepath(this);

    errorMessage = util.format(
      'The scope name "%s" is not recognized. Use one of the following values: %j',
      scope,
      scopeNames,
    );
    if (filepath) {
 ...
 */
function _combine(primary, secondary, target, exclude) {
  var primaryProperties = _.difference(Object.getOwnPropertyNames(primary), exclude);
  var secondaryProperties = _.difference(
 ...
 */
function _combineWithLogic(primary, secondary, target, include) {
...
export function combine(primary, secondary) {
  var specialCase = ['params', 'properties'];
  var target = new Doclet('');

  _combine(primary, secondary, target, specialCase);
  _combineWithLogic(primary, secondary, target, specialCase);
