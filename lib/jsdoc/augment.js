...
import { shorten, combine as combineName, SCOPE } from './name';
import { combine } from './doclet';
...
function reparentDoclet(parent, child) {
  var parts = shorten(child.longname);

  parts.memberof = parent.longname;
  child.memberof = parent.longname;
  child.longname = combineName(parts);
}
...
function staticToInstance(doclet) {
  var parts = shorten(doclet.longname);

  parts.scope = SCOPE.PUNC.INSTANCE;
  doclet.longname = combineName(parts);
...
function changeMemberof(longname, newMemberof) {
  var atoms = shorten(longname);

  atoms.memberof = newMemberof;

  return combineName(atoms);
}
    ...
    if (parents && (doc.kind === 'class' || doc.kind === 'interface')) {
      ...
      for (var j = 0, jj = parents.length; j < jj; j++) {
        ...
        for (var k = 0, kk = parentMembers.length; k < kk; k++) {
          ...
          member = combine(childDoclet, parentDoclet);
          ...
          parts = shorten(member.longname);
          parts.memberof = doc.longname;
          member.longname = combineName(parts);
          ...
          // update the doclets to indicate what the descendant is overriding.
            ...
            addDocletProperty(
              index.documented[member.longname],
              'overrides',
...
function updateMixes(mixedDoclet, mixedLongname) {
  ...
  if (!mixedDoclet.mixes) {
    ...
    mixedName = shorten(mixedLongname).name;
    ...
    names = mixedDoclet.mixes.map(() => shorten(mixedDoclet.longname).name);
    ...
    if (mixes) {
      ...
      for (var j = 0, jj = mixes.length; j < jj; j++) {
        ...
        for (var k = 0, kk = mixedDoclets.length; k < kk; k++) {
          ...
          mixedDoclet = cloneDeep(mixedDoclets[k]);
    ...
    if (implementations) {
      ...
      for (var j = 0, jj = implementations.length; j < jj; j++) {
        ...
        for (var k = 0, kk = interfaceDoclets.length; k < kk; k++) {
          ...
          implementationDoclet = combine(childDoclet, parentDoclet);
