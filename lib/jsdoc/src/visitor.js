...
function getParentDocletFromEvent(parser, e) {
  if (
    ...
    e.doclet.meta &&
    e.doclet.meta.code &&
    e.doclet.meta.code.node &&
    e.doclet.meta.code.node.parent
  ) {
    return parser.getDocletById(e.doclet.meta.code.node.parent.nodeId);
  }
...
function makeConstructorFinisher(parser) {
  return function (e) {
    ...
    if (
      ...
      parentDoclet = parser.getDocletById(e.code.node.parent.parent.parent.nodeId);
    }
    // otherwise, we want the ClassDeclaration
    else {
      parentDoclet = parser.getDocletById(e.code.node.parent.parent.nodeId);
    }
    ...
    combined = combine(doclet, parentDoclet);
...
function trackVars(parser, node, e) {
  ...
  if (enclosingScopeId) {
    doclet = parser.getDocletById(enclosingScopeId);
  } else {
    doclet = parser.getDocletByLongname(name.LONGNAMES.GLOBAL);
  }
...
export default class Visitor {
  ...
  // TODO: docs
  getAstNodeVisitors = () => this._nodeVisitors;
  ...
  // TODO: docs
  makeSymbolFoundEvent = (node, parser, filename) => {
    var e;
    var basename;
    var parent;
