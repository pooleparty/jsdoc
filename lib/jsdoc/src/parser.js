...
export class Parser extends EventEmitter {
  ...
  clear = () => {
    ...
    this.cacheLongname.put(name.LONGNAMES.GLOBAL, {
      meta: {},
    });
  };
  ...
  astnodeToMemberof = (node) => {
    ...
    // confusing...
      ...
      if (doclet) {
        result.memberof =
          doclet.longname +
          (node.static === true ? name.SCOPE.PUNC.STATIC : name.SCOPE.PUNC.INSTANCE);
      }
    } else {
      ...
      doclet = this.getDocletByLongname(name.LONGNAMES.GLOBAL);
      if (doclet && definedInScope(doclet, basename)) {
        result.memberof = doclet.meta.vars[basename];
        result.basename = basename;
      } else {
