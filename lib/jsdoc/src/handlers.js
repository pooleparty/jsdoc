...
import { Doclet } from '../doclet';
import { Syntax } from './syntax';
import { SCOPE } from '../name';
import logger from '../util/logger';

var currentModule = null;
var SCOPE_NAMES = SCOPE.NAMES;
var SCOPE_PUNC = SCOPE.PUNC;
...
function createDoclet(comment, e) {
  ...
  try {
    ...
    err = new Error(util.format(
      'cannot create a doclet for the comment "%s": %s',
      comment.replace(/[\r\n]/g, ''),
      error.message,
    ));
    logger.error(err);
    ...
    if (!doclet.scope) {
      ...
      if (
        ...
        if (doclet.meta.code.node.kind === 'constructor') {
          parentDoclet = parser.getDocletById(doclet.meta.code.node.parent.parent.nodeId);
