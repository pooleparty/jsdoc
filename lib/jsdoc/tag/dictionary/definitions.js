...
function setDocletTypeToValueType(doclet, tag) {
  if (tag.value && tag.value.type) {
    ...
    Object.keys(tag.value).forEach((prop) => {
...
export const jsdocTags = baseTags;
 ...
 */
export function defineTags(dictionary, tagDefinitions) {
  var dictionaries;
      ...
      if (!tagDefs) {
        jsdoc.util.logger.error(
          'The configuration setting "tags.dictionaries" contains ' +
            'the unknown dictionary name %s. Ignoring the dictionary.',
          dictName,
        );
