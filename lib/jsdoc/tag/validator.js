 ...
 */
export function validate(tag, tagDef, meta) {
  ...
  if (!tagDef) {
    ...
    if (
      !allowUnknownTags ||
      (Array.isArray(allowUnknownTags) && allowUnknownTags.indexOf(tag.title) < 0)
    ) {
  ...
  // check for minor issues that are usually harmless
    ...
    logger.warn(buildMessage(tag.title, meta, 'does not permit a value; the value will be ignored'));
