    ...
    case TYPES.TypeApplication:
      ...
      if (isOutermostType) {
        applications = parsedType.applications
          ...
          .join(', ');
        typeString = util.format('%s.<%s>', getTypeStrings(parsedType.expression), applications);
...
function parseTypeExpression(tagInfo) {
  ...
  try {
    ...
    throw new Error(util.format('Invalid type expression "%s": %s', tagInfo.typeExpression, e.message));
  }
