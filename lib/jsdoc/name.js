...
function atomize(longname, sliceChars, forcedMemberof) {
  ...
  if (typeof forcedMemberof !== 'undefined') {
    ...
    parts = (
      longname.match(new RegExp(`^(:?(.+)([${sliceChars.join()}]))?(.+?)$`)) || []
    ).reverse();
    name = parts[0] || '';
    scopePunc = parts[1] || '';
    memberof = parts[2] || '';
 ...
 */
export function shorten(longname, forcedMemberof) {
 ...
 */
export function resolve(doclet) {
  var about = {};
  ...
  if (memberof || doclet.forceMemberof) {
    ...
    if (name && nameIsLongname(name, memberof) && name !== memberof) {
      about = shorten(name, doclet.forceMemberof ? memberof : undefined);
    }
    ...
    // like @memberof foo# or @memberof foo~
      ...
      about = shorten(memberof + name, doclet.forceMemberof ? memberof : undefined);
...
export function applyNamespace(longname, ns) {
  var nameParts = shorten(longname);
 ...
 * @returns {object} Hash with "name" and "description" properties.
 */
export function splitName(nameDesc) {
